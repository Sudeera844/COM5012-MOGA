...
def load_shelves(filename):
    ...
    with open(filename, 'r') as f:
        for line in f:
            number, thickness, position, top_gap, left_gap, inter_gap, right_gap = map(int, line.strip().split())
            shelves.append({
                'number': number,
                'thickness': thickness,
                'position': position,
...
def genetic_algorithm(products, bays, shelves, pop_size, num_generations, mutation_rate, num_parents):
    ...
    for generation in range(num_generations):
        ...
        offspring_fitnesses = [calculate_fitness(child, products, bays, shelves) for child in offspring]

        population, fitnesses = replacement(population, offspring, fitnesses, offspring_fitnesses, pop_size)
...
if __name__ == "__main__":
    ...
    bays = load_bay('baytp1.txt')  # Or 'baytp2.txt'
    ...
    best_solution, best_fitness, best_fitness_history = genetic_algorithm(products, bays, shelves, pop_size,
